import 'package:flutter/material.dart';

class ThemeColor {
  ThemeColor(this.darkTheme);

  static ThemeColor of(bool darkTheme) {
    return ThemeColor(darkTheme);
  }

  final bool darkTheme;

  MaterialColor get backgroundPrimary {
    if (darkTheme) {
      return const MaterialColor(
        0xDD000000,
        <int, Color>{
          50: Colors.black87,
          100: Colors.black87,
          200: Colors.black87,
          300: Colors.black87,
          400: Colors.black87,
          500: Colors.black87,
          600: Colors.black87,
          700: Colors.black87,
          800: Colors.black87,
          900: Colors.black87,
        },
      );
    } else {
      return const MaterialColor(
        0xFFFFFFFF,
        <int, Color>{
          50: Colors.white,
          100: Colors.white,
          200: Colors.white,
          300: Colors.white,
          400: Colors.white,
          500: Colors.white,
          600: Colors.white,
          700: Colors.white,
          800: Colors.white,
          900: Colors.white,
        },
      );
    }
  }

  MaterialColor get backgroundSecondary {
    if (darkTheme) {
      return const MaterialColor(
        0x8A000000,
        <int, Color>{
          50: Colors.black54,
          100: Colors.black54,
          200: Colors.black54,
          300: Colors.black54,
          400: Colors.black54,
          500: Colors.black54,
          600: Colors.black54,
          700: Colors.black54,
          800: Colors.black54,
          900: Colors.black54,
        },
      );
    } else {
      return const MaterialColor(
        0xFFECECEC,
        <int, Color>{
          50: Color(0xFFECECEC),
          100: Color(0xFFECECEC),
          200: Color(0xFFECECEC),
          300: Color(0xFFECECEC),
          400: Color(0xFFECECEC),
          500: Color(0xFFECECEC),
          600: Color(0xFFECECEC),
          700: Color(0xFFECECEC),
          800: Color(0xFFECECEC),
          900: Color(0xFFECECEC),
        },
      );
    }
  }

  MaterialColor get textPrimary {
    if (darkTheme) {
      return const MaterialColor(
        0xFFFFFFFF,
        <int, Color>{
          50: Colors.white,
          100: Colors.white,
          200: Colors.white,
          300: Colors.white,
          400: Colors.white,
          500: Colors.white,
          600: Colors.white,
          700: Colors.white,
          800: Colors.white,
          900: Colors.white,
        },
      );
    } else {
      return const MaterialColor(
        0xFF000000,
        <int, Color>{
          50: Colors.black,
          100: Colors.black,
          200: Colors.black,
          300: Colors.black,
          400: Colors.black,
          500: Colors.black,
          600: Colors.black,
          700: Colors.black,
          800: Colors.black,
          900: Colors.black,
        },
      );
    }
  }

  MaterialColor get textPrimaryHover {
    return const MaterialColor(
      0xFF448AFF,
      <int, Color>{
        50: Colors.blueAccent,
        100: Colors.blueAccent,
        200: Colors.blueAccent,
        300: Colors.blueAccent,
        400: Colors.blueAccent,
        500: Colors.blueAccent,
        600: Colors.blueAccent,
        700: Colors.blueAccent,
        800: Colors.blueAccent,
        900: Colors.blueAccent,
      },
    );
  }

  MaterialColor get textSecondary {
    if (darkTheme) {
      return const MaterialColor(
        0xB3FFFFFF,
        <int, Color>{
          50: Colors.white70,
          100: Colors.white70,
          200: Colors.white70,
          300: Colors.white70,
          400: Colors.white70,
          500: Colors.white70,
          600: Colors.white70,
          700: Colors.white70,
          800: Colors.white70,
          900: Colors.white70,
        },
      );
    } else {
      return const MaterialColor(
        0xDD000000,
        <int, Color>{
          50: Colors.black87,
          100: Colors.black87,
          200: Colors.black87,
          300: Colors.black87,
          400: Colors.black87,
          500: Colors.black87,
          600: Colors.black87,
          700: Colors.black87,
          800: Colors.black87,
          900: Colors.black87,
        },
      );
    }
  }

  MaterialColor get cursor {
    if (darkTheme) {
      return const MaterialColor(
        0xFFFFFFFF,
        <int, Color>{
          50: Colors.white,
          100: Colors.white,
          200: Colors.white,
          300: Colors.white,
          400: Colors.white,
          500: Colors.white,
          600: Colors.white,
          700: Colors.white,
          800: Colors.white,
          900: Colors.white,
        },
      );
    } else {
      return const MaterialColor(
        0xFF9E9E9E,
        <int, Color>{
          50: Colors.grey,
          100: Colors.grey,
          200: Colors.grey,
          300: Colors.grey,
          400: Colors.grey,
          500: Colors.grey,
          600: Colors.grey,
          700: Colors.grey,
          800: Colors.grey,
          900: Colors.grey,
        },
      );
    }
  }

  MaterialColor get selection {
    if (darkTheme) {
      return const MaterialColor(
        0x1AFFFFFF,
        <int, Color>{
          50: Colors.white10,
          100: Colors.white10,
          200: Colors.white10,
          300: Colors.white10,
          400: Colors.white10,
          500: Colors.white10,
          600: Colors.white10,
          700: Colors.white10,
          800: Colors.white10,
          900: Colors.white10,
        },
      );
    } else {
      return const MaterialColor(
        0x1F000000,
        <int, Color>{
          50: Colors.black12,
          100: Colors.black12,
          200: Colors.black12,
          300: Colors.black12,
          400: Colors.black12,
          500: Colors.black12,
          600: Colors.black12,
          700: Colors.black12,
          800: Colors.black12,
          900: Colors.black12,
        },
      );
    }
  }

  MaterialColor get buttonPrimary {
    if (darkTheme) {
      return const MaterialColor(
        0xFF5A5A5A,
        <int, Color>{
          50: Color(0xFF5A5A5A),
          100: Color(0xFF5A5A5A),
          200: Color(0xFF5A5A5A),
          300: Color(0xFF5A5A5A),
          400: Color(0xFF5A5A5A),
          500: Color(0xFF5A5A5A),
          600: Color(0xFF5A5A5A),
          700: Color(0xFF5A5A5A),
          800: Color(0xFF5A5A5A),
          900: Color(0xFF5A5A5A),
        },
      );
    } else {
      return const MaterialColor(
        0xFF0A027C,
        <int, Color>{
          50: Color(0xFF0A027C),
          100: Color(0xFF0A027C),
          200: Color(0xFF0A027C),
          300: Color(0xFF0A027C),
          400: Color(0xFF0A027C),
          500: Color(0xFF0A027C),
          600: Color(0xFF0A027C),
          700: Color(0xFF0A027C),
          800: Color(0xFF0A027C),
          900: Color(0xFF0A027C),
        },
      );
    }
  }

  MaterialColor get buttonText {
    return const MaterialColor(
      0xFFFFFFFF,
      <int, Color>{
        50: Colors.white,
        100: Colors.white,
        200: Colors.white,
        300: Colors.white,
        400: Colors.white,
        500: Colors.white,
        600: Colors.white,
        700: Colors.white,
        800: Colors.white,
        900: Colors.white,
      },
    );
  }

  MaterialColor get buttonSecondary {
    if (darkTheme) {
      return const MaterialColor(
        0xFF3A3A3A,
        <int, Color>{
          50: Color(0xFF3A3A3A),
          100: Color(0xFF3A3A3A),
          200: Color(0xFF3A3A3A),
          300: Color(0xFF3A3A3A),
          400: Color(0xFF3A3A3A),
          500: Color(0xFF3A3A3A),
          600: Color(0xFF3A3A3A),
          700: Color(0xFF3A3A3A),
          800: Color(0xFF3A3A3A),
          900: Color(0xFF3A3A3A),
        },
      );
    } else {
      return const MaterialColor(
        0xFF03002e,
        <int, Color>{
          50: Color(0xFF03002e),
          100: Color(0xFF03002e),
          200: Color(0xFF03002e),
          300: Color(0xFF03002e),
          400: Color(0xFF03002e),
          500: Color(0xFF03002e),
          600: Color(0xFF03002e),
          700: Color(0xFF03002e),
          800: Color(0xFF03002e),
          900: Color(0xFF03002e),
        },
      );
    }
  }

  MaterialColor get buttonBorder {
    return const MaterialColor(
      0x42000000,
      <int, Color>{
        50: Colors.black26,
        100: Colors.black26,
        200: Colors.black26,
        300: Colors.black26,
        400: Colors.black26,
        500: Colors.black26,
        600: Colors.black26,
        700: Colors.black26,
        800: Colors.black26,
        900: Colors.black26,
      },
    );
  }

  MaterialColor get buttonSplash {
    return const MaterialColor(
      0x1F000000,
      <int, Color>{
        50: Colors.black12,
        100: Colors.black12,
        200: Colors.black12,
        300: Colors.black12,
        400: Colors.black12,
        500: Colors.black12,
        600: Colors.black12,
        700: Colors.black12,
        800: Colors.black12,
        900: Colors.black12,
      },
    );
  }

  MaterialColor get alertBoxBackground {
    if (darkTheme) {
      return const MaterialColor(
        0xFF1F1F1F,
        <int, Color>{
          50: Color(0xFF1F1F1F),
          100: Color(0xFF1F1F1F),
          200: Color(0xFF1F1F1F),
          300: Color(0xFF1F1F1F),
          400: Color(0xFF1F1F1F),
          500: Color(0xFF1F1F1F),
          600: Color(0xFF1F1F1F),
          700: Color(0xFF1F1F1F),
          800: Color(0xFF1F1F1F),
          900: Color(0xFF1F1F1F),
        },
      );
    } else {
      return const MaterialColor(
        0xFFFFFFFF,
        <int, Color>{
          50: Colors.white,
          100: Colors.white,
          200: Colors.white,
          300: Colors.white,
          400: Colors.white,
          500: Colors.white,
          600: Colors.white,
          700: Colors.white,
          800: Colors.white,
          900: Colors.white,
        },
      );
    }
  }

  MaterialColor get link {
    return const MaterialColor(
      0xFF448AFF,
      <int, Color>{
        50: Colors.blueAccent,
        100: Colors.blueAccent,
        200: Colors.blueAccent,
        300: Colors.blueAccent,
        400: Colors.blueAccent,
        500: Colors.blueAccent,
        600: Colors.blueAccent,
        700: Colors.blueAccent,
        800: Colors.blueAccent,
        900: Colors.blueAccent,
      },
    );
  }
}
